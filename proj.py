...
class DataProcessor(object):
    ...
    @classmethod
    def _read_csv(cls, file, quotechar=None):
        ds = []
        with open(file, 'r') as fp:
            lines = list(csv.reader(fp))
            # training set: text + class + id
            # testing set: id + text
            ds = lines[1:]
        return ds        

class MyProcessor(DataProcessor):
    ''' My own data processing class'''
    
    def _get_label(self, textLabel):
        label = -10
        if textLabel == 'positive':
            label = 2
        elif textLabel == 'neutral':
            label = 1
        elif textLabel == 'negative':
            label = 0
        else:
            raise RuntimeError('label undefined!')
            
        assert label >= 0
        return label
    
    def _create_examples(self, ds, set_type):
        examples = []
        # postive, neutral, negative ==> 2, 1, 0
        if set_type == 'train' or set_type == 'dev':
            ...
            raise RuntimeError('expr set_type undefined!')
